...
from gesture_detector_test import GestureDetectorTest
...
def initialize_systems():
    ...
    try:
        # Initialize test gesture detector (no ML model required)
        gesture_detector = GestureDetectorTest()
        logger.info("Test gesture detector initialized successfully")
        ...
        if camera.isOpened():
            ...
            logger.warning("Camera not available - will simulate")
            camera = None
...
async def root():
    ...
    return {
        "message": "NeoPilot API (Test Mode)",
        "status": "active",
        "mode": "test",
        "endpoints": {
            "websocket": "/ws",
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            # Simulate or capture frame from camera
            frame = None
            ...
            if gesture_detector is not None and frame is not None:
                try:
            ...
            if frame is not None and landmarks and gesture_detector is not None:
                try:
                    frame = gesture_detector.draw_landmarks(frame, landmarks)
                except Exception as e:
                    logger.error(f"Error drawing landmarks: {e}")
            
            # Add gesture info to frame if available
            if frame is not None and gesture_name:
